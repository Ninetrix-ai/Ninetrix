...
from mcp_gateway.core.auth import verify_token
from mcp_gateway.core import saas_client
...
@router.post("/v1/mcp/{workspace_id}")
async def handle_mcp(
    ...
    Security: the workspace resolved from the Bearer token ALWAYS wins.
    The {workspace_id} URL parameter is ignored — it exists only for
    readability/routing and cannot be used to access another workspace's tools.
    """
...
async def _handle_single(workspace_id: str, body: dict) -> dict:
    req_id = body.get("id")
    ...
    try:
        if method == "initialize":
            ...
            entry = registry.get_worker_for_tool(workspace_id, tool_name)
            if not entry:
            ...
            logger.debug(
                "Routing %s → worker=%s server=%s tool=%s workspace=%s",
                tool_name, conn.worker_id, server_name, local_tool_name, workspace_id,
            )
        ...
        logger.error("Error handling %s for workspace %s: %s", method, workspace_id, exc)
